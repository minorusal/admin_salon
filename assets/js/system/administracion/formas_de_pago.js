...
function load_content(uri, id_content){
    ...
    jQuery.ajax({
        ...
        success: function(data){
        	jQuery('#form_forma_pago_edit').remove();
        	jQuery('#form_forma_pago_save').remove();
           if(id_content==1){
           		var funcion = 'buscar';
...
function actualizar(){
	...
	var objData = formData('#form_forma_pago_edit');
  	objData['incomplete'] = values_requeridos('form_forma_pago_edit');
...
function agregar(){
	...
	var objData = formData('#form_forma_pago_save');
  	objData['incomplete'] = values_requeridos('form_forma_pago_save');
...
function eliminar(id_forma_pago){
	var progress = progress_initialized('loader_global');
    jQuery.ajax({
        type: "POST",
        url: path()+"administracion/formas_de_pago/eliminar_registro",
        dataType: 'json',
        data: {id_forma_pago : id_forma_pago},
        beforeSend: function(){
        },
        success: function(data){
        	if(data.success == 'true'){
        		jQuery('#ico-eliminar_'+data.id_forma_pago).parent().parent().parent().remove();
        		jgrowl(data.mensaje);
        	}else{
...
function confirm_delete(id_forma_pago){
	promp_delete(eliminar,id_forma_pago);
}