...
function eliminar(id_tiempo){
  var progress = progress_initialized('loader_global');
    jQuery.ajax({
        type: "POST",
        url: path()+"nutricion/tiempos/eliminar_registro",
        dataType: 'json',
        data: {id_tiempo : id_tiempo},
        beforeSend: function(){
        },
        success: function(data){
          if(data.success == 'true'){
            jQuery('#ico-eliminar_'+data.id_tiempo).parent().parent().parent().remove();
            jgrowl(data.mensaje);
          }else{
...
function confirm_delete(id_tiempo){
  promp_delete(eliminar,id_tiempo);
}
