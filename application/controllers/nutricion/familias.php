...
class familias extends Base_Controller{
	...
	public function listado($offset=0){
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$familias = $this->db_model->db_get_familia_usada($accion_id);
				//print_debug($ciclo);
				...
				$eliminar = '<span id="ico-eliminar_'.$accion_id.'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$accion_id.')" title="'.$this->lang_item("eliminar").'"></span>';
				$btn_acciones['eliminar']       = ($familias[0]['num_familias'] == 0)?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$accion_id.'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("eliminar").'"></span>';
	...
	public function actualizar(){
		...
		if($objData['incomplete']>0){
			...
			$sqlData = array(
				 'id_nutricion_familia' => $objData['id_familia']
				,'familia'              => $objData['txt_familia']
	...
	public function eliminar_registro(){
		...
		if($familias[0]['num_familias'] == 0){
			$sqlData = array(
					'id_nutricion_familia' => $id_familia
		            ,'activo'              => 0
		            ,'edit_id_usuario'     => $this->session->userdata('id_usuario')
					,'edit_timestamp'      => $this->timestamp()
					);
