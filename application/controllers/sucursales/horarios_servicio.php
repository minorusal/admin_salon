...
class horarios_servicio extends Base_Controller{
	...
	public function listado($offset=0){
		...
		if($total_rows){
			...
			// Evento de enlace
				...
				$servicios = $this->db_model->db_get_servicio_user($value['id_administracion_servicio']);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_'.$value['id_administracion_servicio'].'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_administracion_servicio'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = ($servicios[0]['num_servicios'] == 0)?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_servicio'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
	...
	public function eliminar_registro(){
		$id_servicio = $this->ajax_post('id_servicio');
		$servicios = $this->db_model->db_get_servicio_user($id_servicio);
		if($servicios[0]['num_servicios'] == 0){
			$sqlData = array(
					'id_administracion_servicio' => $id_servicio
		            ,'activo'                    => 0
		            ,'edit_id_usuario'           => $this->session->userdata('id_usuario')
			...
			if($update){
				...
				echo json_encode(array('success'=>'true', 'mensaje' => $msg, 'id_servicio' => $id_servicio));
			}else{
