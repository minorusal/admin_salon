...
class puestos extends Base_Controller{
	private $modulo;
	private $submodulo;
	private $seccion;
	...
	public function __construct(){
		...
		$this->seccion          = 'puestos';
		$this->icon 			= 'fa fa-sitemap'; 
		$this->path 			= $this->modulo.'/'.$this->seccion.'/'; #administracion/puestos
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$atrr = array(
								'href' => '#',
							  	'onclick' => $tab_detalle.'('.$value['id_administracion_puestos'].')'
						);
				$puesto   = $this->db_model->db_get_puesto_user($value['id_administracion_puestos']);
				$eliminar = '<span style="color:red;" id="ico-eliminar_'.$value['id_administracion_puestos'].'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_administracion_puestos'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = (empty($puesto))?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_puestos'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['detalle'] 		= '<span id="ico-detalle_'.$value['id_administracion_puestos'].'" class="ico_acciones ico_detalle fa fa-search-plus" onclick="detalle('.$value['id_administracion_puestos'].')" title="'.$this->lang_item("detalle").'"></span>';
				$acciones = implode('&nbsp;&nbsp;&nbsp;',$btn_acciones);
				// Datos para tabla
				$tbl_data[] = array('id'            => $value['id_administracion_puestos'],
									'puesto'      => tool_tips_tpl($value['puesto'], $this->lang_item("tool_tip"), 'right' , $atrr),
	...
	public function detalle(){
		...
		$detalle = $this->db_model->get_orden_unico_puesto($id_puesto);
		$seccion = $this->tab3;
		$tab_detalle = $this->tab3;
		$sqlData = array(
			 'buscar' => ''
		...
		$tabData['txt_puesto']              = $detalle[0]['puesto'];
	...
	public function actualizar(){
		...
		if($objData['incomplete']>0){
			...
			$sqlData = array(
				 'id_administracion_puestos' => $objData['id_puesto']
				,'puesto'                    => $objData['txt_puesto']
	...
	public function agregar(){
		...
		$tab_1['lbl_puesto']      = $this->lang_item('lbl_puesto');
	...
	public function eliminar_registro(){
		$id_puesto = $this->ajax_post('id_puesto');
		$puesto = $this->db_model->db_get_puesto_user($id_puesto);
		if(!$puesto[0]['id_puesto']){
			$sqlData = array(
					'id_administracion_puestos' => $id_puesto
		            ,'activo'                   => 0
		            ,'edit_id_usuario'          => $this->session->userdata('id_usuario')
					,'edit_timestamp'           => $this->timestamp()
					);
			...
			if($update){
				...
				echo json_encode(array('success'=>'true', 'mensaje' => $msg, 'id_puesto' => $id_puesto));
			}else{
