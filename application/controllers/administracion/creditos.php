...
class creditos extends Base_Controller{
	private $modulo;
	private $submodulo;
	private $seccion;
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$atrr = array(
								'href' => '#',
							  	'onclick' => $tab_detalle.'('.$value['id_administracion_creditos'].')'
						);
				$credito = $this->db_model->db_get_credito($value['id_administracion_creditos']);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_'.$value['id_administracion_creditos'].'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_administracion_creditos'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = (empty($credito))?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_creditos'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['detalle'] 		= '<span id="ico-detalle_'.$value['id_administracion_creditos'].'" class="ico_acciones ico_detalle fa fa-search-plus" onclick="detalle('.$value['id_administracion_creditos'].')" title="'.$this->lang_item("detalle").'"></span>';
				$acciones = implode('&nbsp;&nbsp;&nbsp;',$btn_acciones);
				// Datos para tabla
				$tbl_data[] = array('id'            => $value['id_administracion_creditos'],
									'impuesto'      => tool_tips_tpl($value['credito'], $this->lang_item("tool_tip"), 'right' , $atrr),
									'valor'         => $value['valor_credito'],
	...
	public function detalle(){
        ...
        $tabData['id_credito']             = $id_credito;
        $tabData["lbl_credito"]            = $this->lang_item("lbl_credito");
	...
	public function actualizar(){
		...
		if($objData['incomplete']>0){
			...
			$sqlData = array(
						 'id_administracion_creditos'   => $objData['id_credito']
						,'credito'          		    => $objData['txt_credito']
						,'valor_credito'             	=> $objData['txt_valor']
	...
	{							#administracion/catalogos_generales/creditos/creditos_save
		$seccion       = $this->modulo.'/'.$this->seccion.'/creditos_save';
		$sqlData        = array(
			 'buscar'      	=> ''
			,'offset' 		=> 0
			,'limit'      	=> 0
		...
		//$tab_1["nombre_credito"]    = $this->lang_item("nombre_credito");
		$tab_1["lbl_credito"]       = $this->lang_item("lbl_credito");
	...
	public function insert_credito(){
		$objData  	= $this->ajax_post('objData');
		if($objData['incomplete']>0){
			$msg = $this->lang_item("msg_campos_obligatorios",false);
			echo json_encode( array( 'success'=>'false', 'mensaje' => alertas_tpl('error', $msg ,false)));
		}else{
			$data_insert     = array('credito'     => $objData['txt_credito'],
								 'valor_credito'   => $objData['txt_valor'],
								 'clave_corta'     => $objData['txt_clave_corta'],
								 'descripcion'     => $objData['txt_descripcion'],
...
//https://wrapbootstrap.com/themes/page.1/sort.rank/order.asc
	...
	public function eliminar_registro(){
		$id_credito = $this->ajax_post('id_credito');
		$credito = $this->db_model->db_get_credito($id_credito);
		if(!$credito[0]['id_administracion_creditos']){
			$sqlData = array(
					 'id_administracion_creditos'  => $id_credito
		            ,'activo'                      => 0
		            ,'edit_id_usuario'             => $this->session->userdata('id_usuario')
					,'edit_timestamp'              => $this->timestamp()
					);
			...
			if($update){
				...
				echo json_encode(array('success'=>'true', 'mensaje' => $msg, 'id_credito' => $id_credito));
			}else{
