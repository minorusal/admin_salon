...
class impuestos extends Base_Controller{
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$impuesto = $this->db_model->db_get_impuesto($value['id_administracion_impuestos']);
				//print_debug($impuesto);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_'.$value['id_administracion_impuestos'].'" class="ico-eliminar fa fa-times" onclick="confirm_delete('.$value['id_administracion_impuestos'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = ($impuesto[0]['num_impuestos'] == 0)?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_impuestos'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['detalle'] 		= '<span id="ico-detalle_'.$value['id_administracion_impuestos'].'" class="ico_acciones ico_detalle fa fa-search-plus" onclick="detalle('.$value['id_administracion_impuestos'].')" title="'.$this->lang_item("detalle").'"></span>';
	...
	public function eliminar_registro(){
		$id_impuesto = $this->ajax_post('id_impuesto');
		$impuesto = $this->db_model->db_get_impuesto($id_impuesto);
		if($impuesto[0]['num_impuestos'] == 0){
			$sqlData = array(
					 'id_administracion_impuestos'  => $id_impuesto
		            ,'activo'                        => 0
		            ,'edit_id_usuario'               => $this->session->userdata('id_usuario')
					,'edit_timestamp'                => $this->timestamp()
					);
			...
			if($update){
				...
				echo json_encode(array('success'=>'true', 'mensaje' => $msg, 'id_impuesto' => $id_impuesto));
			}else{
