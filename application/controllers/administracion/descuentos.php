...
class descuentos extends Base_Controller{
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$descuento = $this->db_model->db_get_descuento($value['id_administracion_descuentos']);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_'.$value['id_administracion_descuentos'].'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_administracion_descuentos'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = ($descuento[0]['num_descuentos'] == 0)?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_administracion_descuentos'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['detalle'] 		= '<span id="ico-detalle_'.$value['id_administracion_descuentos'].'" class="ico_acciones ico_detalle fa fa-search-plus" onclick="detalle('.$value['id_administracion_descuentos'].')" title="'.$this->lang_item("detalle").'"></span>';
	...
	public function actualizar(){
		...
		if($objData['incomplete']>0){
			...
			$sqlData = array(
						 'id_administracion_descuentos' => $objData['id_descuento']
						,'descuento'          		    => $objData['txt_descuento']
						,'valor_descuento'             	=> $objData['txt_valor']
	...
	public function insert_descuento(){
		...
		if($objData['incomplete']>0){
			...
			$data_insert     = array('descuento'   => $objData['txt_descuento'],
								 'valor_descuento' => $objData['txt_valor'],
								 'clave_corta'     => $objData['txt_clave_corta'],
								 'descripcion'     => $objData['txt_descripcion'],
	...
	public function eliminar_registro(){
		...
		if($descuento[0]['num_descuentos'] == 0){
			$sqlData = array(
					 'id_administracion_descuentos'  => $id_descuento
		            ,'activo'                        => 0
		            ,'edit_id_usuario'               => $this->session->userdata('id_usuario')
					,'edit_timestamp'                => $this->timestamp()
					);
