...
class subrogacion extends Base_Controller{
	private $modulo;
	private $submodulo;
	private $seccion;
	...
	public function __construct(){
		...
		$this->seccion          = 'subrogacion';
		$this->icon 			= 'iconfa-retweet'; 
	...
	public function listado($offset=0){
		...
		if($total_rows){
			foreach ($list_content as $value){
				$atrr = array(
								'href'    => '#',
							  	'onclick' => $tab_detalle.'('.$value['id_administracion_subrogacion'].')'
						);
				...
				$tbl_data[] = array('id'            => $value['id_administracion_subrogacion'],
									'area'          => tool_tips_tpl($value['subrogacion'], $this->lang_item("tool_tip"), 'right' , $atrr),
									'valor'         => $value['valor'],
	...
	public function actualizar(){
		...
		if($objData['incomplete']>0){
			...
			$sqlData = array(
				'id_administracion_subrogacion' => $objData['id_subrogacion']
				,'subrogacion'                  => $objData['txt_subrogacion']
	...
	public function agregar(){
		...
		$tab_1['lbl_subrogacion']  = $this->lang_item("lbl_subrogacion");
	...
	public function export_xlsx($offset=0){
		...
		$params = array(	'title'   => $this->lang_item("lbl_subrogacion"),
							'items'   => $set_data,
							'headers' => $set_heading
						);
		//print_debug($params);
	...
	public function eliminar_registro(){
		...
		if(empty($subrogaciones)){
			$sqlData = array(
					'id_administracion_subrogacion' => $id_subrogacion
		            ,'activo'                       => 0
		            ,'edit_id_usuario'              => $this->session->userdata('id_usuario')
					,'edit_timestamp'               => $this->timestamp()
					);
