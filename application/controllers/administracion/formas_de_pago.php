...
class formas_de_pago extends Base_Controller
{
	...
	// Crea tabla con listado de elementos capturados 
		...
		if($total_rows){
			foreach ($list_content as $value){
				...
				$forma_pago = $this->db_model->db_get_forma_pago($value['id_forma_pago']);
				//print_debug($forma_pago);
				$forma_pago_sucursal =$this->db_model->db_get_forma_pago_sucursal($value['id_forma_pago']);
				$eliminar 	= '<span style="color:red;" id="ico-eliminar_'.$value['id_forma_pago'].'" class="ico_eliminar fa fa-times" onclick="confirm_delete('.$value['id_forma_pago'].')" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['eliminar'] = ($forma_pago[0]['num_fpagos'] == 0)?$eliminar:'<span style="color:gray;" id="ico-eliminar_'.$value['id_forma_pago'].'" class="ico_eliminar fa fa-times" title="'.$this->lang_item("lbl_eliminar").'"></span>';
				$btn_acciones['detalle'] 		= '<span id="ico-detalle_'.$value['id_forma_pago'].'" class="ico_acciones ico_detalle fa fa-search-plus" onclick="detalle('.$value['id_forma_pago'].')" title="'.$this->lang_item("detalle").'"></span>';
	...
	public function eliminar_registro(){
		$id_forma_pago = $this->ajax_post('id_forma_pago');
		$forma_pago = $this->db_model->db_get_forma_pago($id_forma_pago);
		//$forma_pago_sucursal =$this->db_model->db_get_forma_pago_sucursal($id_forma_pago);
		if($forma_pago[0]['num_fpagos'] == 0){
			$sqlData = array(
			...
			if($update){
				...
				echo json_encode(array('success'=>'true', 'mensaje' => $msg, 'id_forma_pago' => $id_forma_pago));
			}else{
